...
class Arvore_Binaria:
    ...
    def mostrar_pre_ordem(self):
        ...
        if self.raiz is None:
            ...
            self._mostrar_pre_ordem_recursiva(self.raiz, valores)
        return valores

    def _mostrar_pre_ordem_recursiva(self, no, valores):
        if no is not None:
            valores.append(no.valor)
            self._mostrar_pre_ordem_recursiva(no.esquerda, valores)
            self._mostrar_pre_ordem_recursiva(no.direita, valores)
...
valores_pre_ordem = arvore.mostrar_pre_ordem()
print("\nValores Pré-Ordem")
print(valores_pre_ordem)
